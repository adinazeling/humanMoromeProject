...
icd10_df = icd10_select("subgroup")
...
age_sex_nest = age_sex_nest %>% 
  ...
  mutate(count = map(.x = data, ~count_grp(.x, sub_chapter_group)),
         count_fall = map(.x = tidy, ~count_fall(.x, sub_chapter_group))) %>% 
...
# Add missing points for diseases 
age_plot = age_plot %>% 
  add_row(age_grp = "[80,85)", sex = "F", icd10 = "K70", par = 0) %>% 
  add_row(age_grp = "[85,90)", sex = "F", icd10 = "K70", par = 0) %>% 
  add_row(age_grp = "[90,120)", sex = "F", icd10 = "K70", par = 0) %>% 
  add_row(age_grp = "[65,70)", sex = "F", icd10 = "R54", par = 0) %>% 
  add_row(age_grp = "[90,120)", sex = "F", icd10 = "T51", par = 0) %>% 
  add_row(age_grp = "[90,120)", sex = "M", icd10 = "T51", par = 0) %>% 
  add_row(age_grp = "[90,120)", sex = "M", icd10 = "X45", par = 0) %>% 
  add_row(age_grp = "[90,120)", sex = "F", icd10 = "X45", par = 0)
...
p = plot2 %>% 
  ...
  facet_grid(.~sex) + 
  labs(title = "Population attributed ratio (PAR) of comorbid conditions associated with deaths from falls in adults 65+ years by sex in 2017",
       x = "Age Group (Years)",
       y = "Population Attributed Ratio (PAR)") +
  geom_text_repel(aes(x = age_grp, y = par, label = label, color = icd10),
                  show.legend = FALSE, size = 5) + 
  scale_y_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12)) + 
  viridis::scale_color_viridis(discrete = TRUE, name = "Disease", 
                      labels = c("Diabetes Mellitus, unspecified (E14)",
                                 ...
                                 "Accidental Poisoning by Alcohol (X45)")) +
  theme_minimal() + 
